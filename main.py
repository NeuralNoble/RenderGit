...
Supports: source files, images (inline), Markdown, Jupyter notebooks (.ipynb).
"""
...
def git_clone(url: str, dst: str) -> None:
    try:
        ...
        print(f"✗  git clone failed (exit {e.returncode}):", file=sys.stderr)
        if msg:
            print(f"   {msg}", file=sys.stderr)
        if "Repository not found" in msg or "does not exist" in msg:
            print("   → The repo URL may be wrong or the repo is private.", file=sys.stderr)
        elif "Could not resolve host" in msg or "unable to access" in msg:
            print("   → Network error. Check your internet connection.", file=sys.stderr)
        elif "already exists" in msg:
            print("   → Destination directory already exists.", file=sys.stderr)
        sys.exit(1)
...
def decide_file(path: pathlib.Path, repo_root: pathlib.Path, max_bytes: int) -> FileInfo:
    ...
    if ext in IMAGE_EXTENSIONS:
        ...
        return FileInfo(path, rel, size, RenderDecision(True, "image"))

    if ext in SVG_EXTENSIONS:
        if size > max_bytes:
            return FileInfo(path, rel, size, RenderDecision(False, "too_large"))
        return FileInfo(path, rel, size, RenderDecision(True, "svg"))

    if ext in NOTEBOOK_EXTENSIONS:
        # Allow notebooks up to 10× the normal text limit (they embed lots of output)
        nb_limit = max(max_bytes * 10, 5 * 1024 * 1024)
        if size > nb_limit:
            return FileInfo(path, rel, size, RenderDecision(False, "too_large"))
        return FileInfo(path, rel, size, RenderDecision(True, "notebook"))
...
def try_tree_command(root: pathlib.Path) -> str:
    try:
        return run(["tree", "-a", "."], cwd=str(root)).stdout
    except Exception:
...
def _output_plain_text(output: Dict[str, Any]) -> str:
    """Pull the best plain-text representation out of an output block."""
    # stream outputs have a "text" key directly
    text = output.get("text")
    if text is not None:
        return _join_lines(text)
    # display_data / execute_result have a data dict
...
def _output_image_html(output: Dict[str, Any]) -> str:
    """Return an <img> or inline SVG if the output contains image data, else ''."""
    data = output.get("data", {})
    for mime in ("image/png", "image/jpeg", "image/gif", "image/svg+xml"):
        ...
        uri = b64_to_data_uri(b64, mime)
        return f'<div class="nb-output-img"><img src="{uri}" style="max-width:100%;" alt="cell output" /></div>'
    return ""


# Map kernel language names to filenames that Pygments can identify
...
def render_notebook(path: pathlib.Path, formatter: HtmlFormatter) -> str:
    """
    Parse a .ipynb file and return HTML representing all cells.
    Raises ValueError on hard parse failure so callers can show a fallback.
    """
    try:
    ...
    for idx, cell in enumerate(cells):
        ...
        # ── Code cell ──────────────────────────────────────────────────────
            ...
            for out in cell.get("outputs", []):
                ...
                # Image output (check before text so rich outputs show the image)
                ...
                if out_type == "error":
                    ...
                    tb     = ANSI_RE.sub("", _join_lines(tb_raw))
                    inner.append(
...
def notebook_to_plain_text(path: pathlib.Path) -> str:
    """
    Extract text content from a notebook for LLM consumption.
    Images in outputs are replaced by a short placeholder — never embedded.
    """
    try:
        nb = json.loads(path.read_bytes())
    except Exception as e:
    ...
    parts: List[str] = [f"# Jupyter Notebook  (kernel: {lang})\n"]
    ...
    for idx, cell in enumerate(nb.get("cells", [])):
        ...
        if cell_type == "markdown":
            ...
            for out in cell.get("outputs", []):
                out_type = out.get("output_type", "")
                # Never include image data in LLM text
                data = out.get("data", {})
...
# ---------------------------------------------------------------------------
# CXML for LLM view  (text-only; images and true binaries excluded)
# ---------------------------------------------------------------------------
...
def generate_cxml_text(infos: List[FileInfo], repo_dir: pathlib.Path) -> str:
    ...
    for idx, fi in enumerate(candidates, 1):
        ...
        try:
            if fi.decision.reason == "notebook":
                lines.append(notebook_to_plain_text(fi.path))
            else:
                lines.append(read_text(fi.path))
        except Exception as e:
...
def build_html(repo_url: str, repo_dir: pathlib.Path, head_commit: str, infos: List[FileInfo]) -> str:
    ...
    # README: prefer shallowest; among equal depth, alphabetical
    ...
    for fi in rendered:
        if readme_info and fi.rel == readme_info.rel:
            continue  # README has its own tab
        ...
        try:
            ...
            if reason == "image":
                ...
                try:
                    nb_html   = render_notebook(fi.path, formatter)
                    body_html = f'<div class="nb-container">{nb_html}</div>'
                except Exception as e:
                    # Graceful fallback: show highlighted raw JSON
                    body_html = (
                        f'<div class="nb-render-warn">'
                        f'⚠️ Notebook render failed — {html.escape(str(e))}. Showing raw JSON.</div>'
    ...
    def render_skip_list(title: str, items: List[FileInfo]) -> str:
        ...
        return (
            ...
            f"<ul class='skip-list'>{lis}</ul></details>"
        )
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; }}
  body {{
    ...
    margin: 0; padding: 0; line-height: 1.5; background: #fff; color: #1f2328;
    font-size: 15px;
  }}
  ...
  .markdown-body h1,
  .markdown-body h2,
  .markdown-body h3 {{ border-bottom: 1px solid #eaecef; padding-bottom: 0.3em; margin-top: 1.2em; }}
  ...
  .markdown-body th,
  .markdown-body td {{ border: 1px solid #d1d9e0; padding: 6px 13px; }}
  ...
  .nb-cell-markdown {{
    padding: 0.6rem 1rem; background: #fff; border-color: transparent;
  }}
  ...
  .nb-output {{
    ...
    border-top: 1px solid #eaecef; background: #fafbfc;
  }}
  ...
  .nb-label {{
    ...
    color: #57606a; text-align: right; user-select: none;
    flex-shrink: 0; border-right: 2px solid #eaecef;
  }}
  ...
  .nb-out-text {{
    flex: 1; margin: 0; padding: 0.55rem 0.85rem;
  ...
  .nb-traceback {{
    flex: 1; margin: 0; padding: 0.55rem 0.85rem;
    background: transparent; border: none;
    font-size: 0.88rem; line-height: 1.5; color: #b00020;
  ...
  #llm-textarea {{
    ...
    border: 1px solid #d1d9e0; border-radius: 6px; padding: 0.75rem;
  }}
  .copy-btn {{
...
<div class="page">
    ...
    <div class="sidebar-inner">
      ...
      <ul class="toc toc-root">
        <li style="padding-bottom:0.4rem">
          <a href="#top" style="color:#57606a;font-size:0.85rem">↑ Top</a>
        </li>
        {toc_html}
      </ul>
    </div>
  ...
  <main>
    <div class="meta">
      ...
      <div class="counts">
        {total_files} total · {len(rendered)} rendered · {len(skipped_bin) + len(skipped_large)} skipped
      </div>
    </div>
    ...
    <!-- README view (only if README exists) -->
    {readme_view_div}
...
function showView(name, btn) {{
  ...
  if (name === 'llm') {{
    const ta = document.getElementById('llm-textarea');
    ta.focus(); ta.select();
  }}
}}

// Sidebar TOC links always live in the human view — switch to it first
document.querySelectorAll('#sidebar a[href^="#file-"]').forEach(link => {{
  link.addEventListener('click', function(e) {{
    ...
    const isHumanVisible = humanPane.style.display !== 'none';
    if (!isHumanVisible) {{
      // Switch to human tab without affecting the hash navigation
      showView('human', humanBtn);
      // Let the browser handle the scroll after a brief paint delay
      e.preventDefault();
...
// Auto-expand sidebar dirs when landing on a #hash
(function() {{
...
def main() -> int:
    ...
    ap.add_argument("-o", "--out", help="Output HTML file (default: system temp dir)")
    ap.add_argument("--max-bytes", type=int, default=MAX_DEFAULT_BYTES,
                    help=f"Max text-file size to render, bytes (default {MAX_DEFAULT_BYTES})")
    ap.add_argument("--no-open", action="store_true", help="Don't open in browser after generation")
    args = ap.parse_args()
    ...
    try:
        ...
        print(f"✓   Clone done (HEAD: {head[:8]})", file=sys.stderr)
        ...
        nb_count       = sum(1 for i in infos if i.decision.reason == "notebook")
        extra          = f" (incl. {nb_count} notebook{'s' if nb_count != 1 else ''})" if nb_count else ""
        print(f"✓   {len(infos)} files · {rendered_count} rendered{extra}", file=sys.stderr)

        print("🔨  Generating HTML ...", file=sys.stderr)
        html_out = build_html(args.repo_url, repo_dir, head, infos)
        out_path = pathlib.Path(args.out)
        out_path.write_text(html_out, encoding="utf-8")
        print(f"✓   Wrote {bytes_human(out_path.stat().st_size)} → {out_path.resolve()}", file=sys.stderr)
